...
public class StarDisplay : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        LoadUserStats();
        Analytics.TrackEvent("Game Played", new Dictionary<string, string> {
            { "user", PlayerPrefs.GetString("PlayerName") },
            { "score", PlayerPrefs.GetInt("Score", 0).ToString() },
            { "accuracy", PlayerPrefs.GetFloat("Accuracy", 0f).ToString() },
            { "rate", PlayerPrefs.GetFloat("Rate", 0f).ToString() },
            { "wrong", PlayerPrefs.GetInt("Wrong", 0).ToString() }
        });
